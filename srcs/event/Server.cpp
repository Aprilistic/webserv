...
Server::Server(EventMonitor *Monitor, Node *ServerNode)
    : mMonitor(Monitor), mServerNode(ServerNode) {
  ...
  kevent(mMonitor->mKqueue, events, 2, NULL, 0, NULL);

  struct timespec *timeout = NULL; // wait indefinitely
  int n = kevent(mMonitor->mKqueue, NULL, 0, events, 1, timeout);
}
