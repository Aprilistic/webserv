...
void Node::CheckClientMaxBodySize(std::vector<std::string> &value) {
  if (value.size() != 1) {
    nodeError("Error: Incorrect number of arguments. Exactly one argument is required.");
  }
  int multiplier;
...
void Node::CheckReturn(std::vector<std::string> &value)
{
	if (value.size() != 2)
	{
    nodeError("Error: Incorrect number of arguments. Exactly two arguments are required.");  
	}
	...
	if (num < 0 || num > 999)
	{
		nodeError("Error: The first value must be a number within the range 0-999.");
	}
}
